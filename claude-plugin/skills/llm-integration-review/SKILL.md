...
agents for security defects, classified against the
...
The model is not a trust boundary: anything in its context can steer it, and
no prompt wording reliably prevents that. Judge each integration by **what the
model's output can reach**, and check that deterministic code, not the prompt,
gates each sink. An agent that ingests untrusted content, sees sensitive data,
*and* can act externally is high-risk by construction (the "Rule of Two"
LLM01:2026 cites).
...
5. If the diff touches no LLM integration, say so and stop
...
- Untrusted content (retrieved documents, tickets, web pages, files, tool or
  MCP results) interpolated into the system prompt or instructions instead of
  passed as separate, labeled data.
...
- No human approval before high-impact, irreversible, or external actions
  (send, delete, pay, post) — the MCP tools spec wants a human able to deny
  tool invocations.
- Authorization left to the model rather than enforced in code at the tool or
  downstream system.
...
- Tool descriptions and annotations from untrusted servers treated as
  trusted — they can carry hidden instructions (tool poisoning, LLM01).
- Unpinned or unvetted MCP server or tool packages, including local servers
  launched from config (LLM04); local servers added without showing the exact
  command for consent, or run unsandboxed, and local HTTP servers without an
  authorization token — prefer `stdio` (LLM03).
...
sink, quote the offending line in the Issue, and drop anything without a
concrete path. Prefer the few findings that matter — if more than ~10 survive,
