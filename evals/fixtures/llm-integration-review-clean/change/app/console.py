...
from triage import suggest_tags
...
@app.post("/tickets/<int:ticket_id>/suggested-tags")
@require_staff
@limiter.limit("20 per minute")
def suggested_tags(ticket_id):
    # Suggestions only: the agent applies each one through add_tag above.
    ticket = get_ticket(ticket_id)
    if ticket is None:
        abort(404)
    return jsonify({"suggested": suggest_tags(ticket)})
