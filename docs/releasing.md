...
2. Run `python scripts/prepare_release.py x.y.z`. It bumps `pyproject.toml`,
   ...
   It validates everything first (version format, a version newer than the
   current one, a non-empty `[Unreleased]`, no existing section for the
   version) and writes nothing if any check fails; if `uv lock` fails it
   restores `pyproject.toml` and `CHANGELOG.md` and exits non-zero.
...
5. Push a tag matching the version: `git tag vX.Y.Z && git push origin vX.Y.Z`.
   ...
   - refuses to continue unless the tagged commit is reachable from `main`
     (branch protection does not cover tags);
...
The release workflow checks what it can, but who may push a release tag and who
must approve the PyPI upload are repository settings. Configure both, and
re-check them whenever maintainers change:
...
- on a tag push, the tag (minus the `v`) **==** the `pyproject` version — run by
  ...
  `refs/tags/vX.Y.Z` is accepted; anything else (for example
  `refs/tags/x/v0.3.0` or `v0.3.0rc1`) is rejected outright.
